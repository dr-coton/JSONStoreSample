"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.writeFileAtomic = exports.Model = void 0;
const tslib_1 = require("tslib");
const combine_errors_1 = tslib_1.__importDefault(require("combine-errors"));
const imurmurhash_1 = tslib_1.__importDefault(require("imurmurhash"));
const signal_exit_1 = tslib_1.__importDefault(require("signal-exit"));
const Model = tslib_1.__importStar(require("./model"));
exports.Model = Model;
const constants_1 = require("../constants");
const defaultAtomicOptions = {
    fsync: true,
    disableChmod: true,
    disableChown: true,
};
const generateTmpFileName = (() => {
    let getTmpFilePathInvocation = 0;
    return (file) => file + "." + imurmurhash_1.default(__filename)
        .hash(String(process.pid))
        .hash(String(++getTmpFilePathInvocation))
        .hash(String(Date.now()))
        .result();
})();
async function writeFileAtomic(fs, filePath, data, writeFileOptions, atomicOptionsInput) {
    const atomicOptions = Object.assign(Object.assign({}, defaultAtomicOptions), atomicOptionsInput);
    const file = filePath.toString();
    const cleanup = { threw: true };
    try {
        // in order to reduce the same file locking probability renaming occurs in serial mode using "queue" approach
        // locking leads to the "EPERM" errors on Windows https://github.com/isaacs/node-graceful-fs/pull/119
        const tmpFile = await (async () => {
            let fileStats;
            try {
                fileStats = await fs.stat(file);
            }
            catch (error) {
                if (error.code !== constants_1.FS_ERROR_CODE_ENOENT) {
                    throw error;
                }
            }
            const resultFile = generateTmpFileName(fileStats ? await fs.realpath(file) : file);
            cleanup.removeOnExitHandler = signal_exit_1.default(cleanup.unlinkTmpFileSync = () => {
                try {
                    fs.unlinkSync(tmpFile);
                }
                catch (_) {
                    // NOOP
                }
            });
            const fd = await fs.open(resultFile, "w");
            try {
                await fs.writeFile(resultFile, data, writeFileOptions);
                if (atomicOptions.fsync) {
                    await fs.fsync(fd);
                }
            }
            finally {
                try {
                    await fs.close(fd);
                }
                catch (_) {
                    // the "fd" might be already closed by error
                }
            }
            if (fileStats) {
                if (!atomicOptions.disableChown) {
                    await fs.chown(resultFile, fileStats.uid, fileStats.gid);
                }
                if (!atomicOptions.disableChmod) {
                    await fs.chmod(resultFile, fileStats.mode);
                }
            }
            return resultFile;
        })();
        try {
            return await fs.rename(tmpFile, file);
        }
        catch (renameError) {
            const errors = [
                renameError,
                new Error(`Failed to rename "${tmpFile}" => "${file}".`),
            ];
            try {
                await fs.unlink(tmpFile);
            }
            catch (_) {
                // also gets unlinked in "finally" block
            }
            throw combine_errors_1.default(errors);
        }
        cleanup.threw = false;
    }
    finally {
        const { threw, removeOnExitHandler, unlinkTmpFileSync } = cleanup;
        if (removeOnExitHandler) {
            removeOnExitHandler();
        }
        if (threw && unlinkTmpFileSync) {
            unlinkTmpFileSync();
        }
    }
}
exports.writeFileAtomic = writeFileAtomic;
//# sourceMappingURL=index.js.map